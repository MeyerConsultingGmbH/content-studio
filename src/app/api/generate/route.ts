...
export async function POST(req: NextRequest) {
  try {
    const { imageBase64, customer, customPrompt, textOnly } = await req.json()
...
${customPrompt ? `\nBESONDERE ANWEISUNGEN (unbedingt beachten):\n${customPrompt}` : ''}
...
{"ig":"...","fb":"...","tags":["tag1","tag2",...]}`
    ...
    userContent.push({
      type: 'text',
      text: textOnly
        ? 'Passe die Texte wie in den Anweisungen beschrieben an. Antworte nur mit JSON.'
        : 'Analysiere dieses Bild und erstelle: 1) Instagram-Text (2-4 Absätze, emotional, 1-2 Emojis) 2) Facebook-Text (etwas informativer) 3) Genau 22 Hashtags. Antworte nur mit JSON.'
    })
